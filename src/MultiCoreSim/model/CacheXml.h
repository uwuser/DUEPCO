...
private:
  ...
  int m_cachePreload;
  int m_latency;                          // Modified to Accomodate Multi Shared Cache
...
public:
  ...
  void LoadFromXml(TiXmlHandle root) {
     ...
     m_nPendingReq     = 1;
     m_cachePreload    = 0;
     m_latency         = 10;                                                            // Modified to Accomodate Multi Shared Cache
     ...
     CacheRootPtr->QueryIntAttribute   ("cachelatency"   , &m_latency         );       // Modified to Accomodate Multi Shared Cache
  }
