...
namespace ns3
{
    ...
    bool BusArbiter::CheckPendingResp(BusIfFIFO::BusRespMsg &txResp, bool ChkOnly, bool oldest)
    {
     ...
     *** equals to RR order requestor then go to the resp buffer and find it   ***/
            ...
            for (unsigned int itr = 0; itr < GlobalQueueSize && PendingTxResp == false; itr++)
            {
                ...
                if (m_ServQueueMsg.reqCoreId == m_respCoreCnt)
                {
                    ...
                    for (int i = 0; i < pendingQueueSize && PendingTxResp == false; i++)
                    {
                        ...
                        if (m_reza_log)
                            cout << " REQ ID " << pendingWbMsg.reqCoreId << "  addr  " << pendingWbMsg.addr << "   RESP ID " << pendingWbMsg.respCoreId << endl;
                        if (pendingWbMsg.reqCoreId == m_respCoreCnt && (pendingWbMsg.addr == m_ServQueueMsg.addr))
                        {
                            if (m_reza_log)
        ...
        if (PendingTxResp == true && ChkOnly == false)
        {
            if (m_logFileGenEnable)
            {
        ...
        return PendingTxResp;
    }

    bool BusArbiter::CheckPendingWriteBackResp(bool CheckOnly, BusIfFIFO::BusRespMsg &txResp, bool oldest)
    {
        /***   if in global oldest there is a msg whcih is PutM that and its wcCoreID equals to RR order requestor then go to the resp buffer and find it   ***/
        /***   if not go check the non oldest **/

        /*** The firs implementation has the issue of having responsed from other cores 
       * but there is nothing in the RR order queue. Therefore, it should be changed
       * such that in every requestor slot, we need to check if there is a "locally
       * oldest" response in the TX response of the other cores for the core that
       * its slot arrived. If such response exists, it should be arbitrated    ***/

        if (m_reza_log)
            cout << "In CheckPendingWriteBackResp " << endl;
         ...
         * other cores and see if there is any response related to this specific request (with msgID) ***/
                ...
                if (m_ServQueueMsg.msgId != 0 && m_ServQueueMsg.reqCoreId == m_respCoreCnt)
                {
                    ...
                    for (std::list<Ptr<BusIfFIFO>>::iterator it4 = m_busIfFIFO.begin(); it4 != m_busIfFIFO.end(); it4++)
                    {
                        ...
                        //cout<<"Check in "<<(*it4)->m_fifo_id<<"  size is "<<pendingQueueSize<<endl;
                        for (int i = 0; i < pendingQueueSize && PendingTxWBResp == false; i++)
                        {
                            pendingWbMsg = (*it4)->m_txRespFIFO.GetFrontElement();
                            (*it4)->m_txRespFIFO.PopElement();
                            //cout<<"in the TX resp the reqID "<<pendingWbMsg.reqCoreId<<"  respID "<<pendingWbMsg.respCoreId<<"  msgID  "<<pendingWbMsg.msgId<<endl;

                            if (((pendingWbMsg.addr >> (int)log2(m_cacheBlkSize)) == (m_ServQueueMsg.addr >> (int)log2(m_cacheBlkSize))) && PendingTxWBResp == false && pendingWbMsg.msgId == m_ServQueueMsg.msgId)
                            {

                                if (CheckOnly == true)
                                {
                                    (*it4)->m_txRespFIFO.InsertElement(pendingWbMsg);
                                }
                    ...
                    /* The following is wrong since the cache to cache might enable some write back to the share cache 
             but you dont arbitrate because the order is not equal to the respCoreID of the actual request.  
          std::advance(it1, m_respCoreCnt);
          int pendingQueueSize = (*it1)->m_txRespFIFO.GetQueueSize();
         if(m_reza_log)  cout<<"pendingQueueSize is "<<pendingQueueSize<<endl;
          ...
          for (int i = 0; i < pendingQueueSize && PendingTxWBResp == false;i++) {
            ...
            pendingWbMsg = (*it1)->m_txRespFIFO.GetFrontElement();
           // cout<<"4"<<endl;
           if(m_reza_log)  cout<<"the write back msg itself has respCoreID "<<pendingWbMsg.respCoreId<<"  and reqcoreID  "<<pendingWbMsg.reqCoreId<<endl;
            // Remove message from the busResp buffer
            (*it1)->m_txRespFIFO.PopElement();

            if (((pendingWbMsg.addr >> (int) log2(m_cacheBlkSize)) == (m_ServQueueMsg.addr >> (int) log2(m_cacheBlkSize))) && 
                PendingTxWBResp == false && pendingWbMsg.respCoreId == m_respCoreCnt) {              
              //cout<<"55"<<endl;
              if (CheckOnly == true) {
               // cout<<"6"<<endl;
                (*it1)->m_txRespFIFO.InsertElement(pendingWbMsg);
         ...
         * this specific core and see if there is any response related to this specific request (with msgID) ***/
                    ...
                    for (int i = 0; i < pendingQueueSize && PendingTxWBResp == false; i++)
                    {
                        ...
                        pendingWbMsg = (*it7)->m_txRespFIFO.GetFrontElement();
                        // cout<<"4"<<endl;
                        if (m_reza_log)
                            cout << "the write back msg itself has reqCoreID " << pendingWbMsg.reqCoreId << "  and respCoreID  " << pendingWbMsg.respCoreId << endl;
                        // Remove message from the busResp buffer
                        (*it7)->m_txRespFIFO.PopElement();

                        if (((pendingWbMsg.addr >> (int)log2(m_cacheBlkSize)) == (m_ServQueueMsg.addr >> (int)log2(m_cacheBlkSize))) && PendingTxWBResp == false && pendingWbMsg.respCoreId == m_respCoreCnt)
                        {
                            if (CheckOnly == true)
                            {
                                (*it7)->m_txRespFIFO.InsertElement(pendingWbMsg);
                            }
                            PendingTxWBResp = true;
                            txResp = pendingWbMsg;
                        }
        ...
        if (PendingTxWBResp == false && m_cach2Cache == true && !oldest)
        {
            if (m_reza_log)
                cout << "No one is chosed by the RR order now lets check the WB from cores that need to be sent to other cores" << endl;

            BusIfFIFO::BusRespMsg pendingWbMsg_temp;

            bool PendingTxWBResp_temp = false;
            int queusize = 0;

            for (std::list<Ptr<BusIfFIFO>>::iterator it2 = m_busIfFIFO.begin(); it2 != m_busIfFIFO.end() && PendingTxWBResp_temp == false; it2++)
            {
                if (m_reza_log)
                    cout << "in outer loop and size of RR queueu " << m_GlobalQueue->m_GlobalRRQueue.size() << "  the id of core is  " << (*it2)->m_fifo_id << endl;
                for (unsigned int RR_iterator = 0; RR_iterator < m_GlobalQueue->m_GlobalRRQueue.size(); RR_iterator++)
                {
                    ...
                    //if(m_reza_log)  cout<<"checking the m_respCoreCnt  "<<m_respCoreCnt<<endl;
                    //std::advance(it3, m_respCoreCnt);
                    if ((*it2)->m_fifo_id != m_respCoreCnt)
                    {

                        if (!((*it2)->m_txRespFIFO.IsEmpty()))
                        {
                            queusize = (*it2)->m_txRespFIFO.GetQueueSize();

                            for (int i = 0; i < queusize && PendingTxWBResp_temp == false; i++)
                            {
                                pendingWbMsg_temp = (*it2)->m_txRespFIFO.GetFrontElement();
                                if (m_reza_log)
                                    cout << "queue size is  " << queusize << "  pendingWbMsg_temp.reqCoreId  " << pendingWbMsg_temp.reqCoreId << "  m_respCoreCnt  " << m_respCoreCnt << endl;
                                (*it2)->m_txRespFIFO.PopElement();

                                if (pendingWbMsg_temp.reqCoreId == m_respCoreCnt)
                                {
                                    cout << "se  " << (*it2)->m_fifo_id << endl;
                                    abort();
                                    if (CheckOnly == true)
                                    {

                                        (*it2)->m_txRespFIFO.InsertElement(pendingWbMsg_temp);
                                    }
                                    if (m_reza_log)
                                        cout << "Agreed to sent the WB from respID " << pendingWbMsg_temp.respCoreId << "  to the reqID  " << pendingWbMsg_temp.reqCoreId << "   ID  " << pendingWbMsg_temp.msgId << "   address  " << pendingWbMsg_temp.addr << endl;
                                    PendingTxWBResp_temp = true;
                                    PendingTxWBResp = true;
                                    txResp = pendingWbMsg_temp;
                                }
                                else
                                {
                                    (*it2)->m_txRespFIFO.InsertElement(pendingWbMsg_temp);
        ...
        /*
    if(PendingTxWBResp == false) {
      cout<<"No one is chosed by the RR order now lets check the WB from cores that need to be sent to other cores"<<endl;
      bool con = false;
      BusIfFIFO::BusRespMsg pendingWbMsg_temp;
      std::list<Ptr<BusIfFIFO>>::iterator it3 = m_busIfFIFO.begin();   
      bool PendingTxWBResp_temp = false;
      int queusize = 0;
      for(std::list<Ptr<BusIfFIFO>>::iterator it2 = m_busIfFIFO.begin(); it2 != m_busIfFIFO.end() && PendingTxWBResp_temp == false; it2++) {
        cout<<"in outer loop and size of RR queueu "<<m_GlobalQueue->m_GlobalRRQueue.size()<<"  the id of core is  "<<(*it2)->m_fifo_id<<endl;
        con = false;
        for(unsigned int RR_iterator = 0; RR_iterator < m_GlobalQueue->m_GlobalRRQueue.size(); RR_iterator++) {
        if(m_GlobalQueue->m_GlobalRRQueue.size() != 0) { m_respCoreCnt = m_GlobalQueue->m_GlobalRRQueue.at(RR_iterator);}
        cout<<"checking the m_respCoreCnt  "<<m_respCoreCnt<<endl;
        std::advance(it3, m_respCoreCnt);
        if(it2 == it3) {
          cout<<"There is something in the RR; therefore we are not processing here"<<endl;
          con = true;
          }
          else{
            cout<<"--------------------"<<endl;
        ...
        if(con == false) {
          ...
          if(!((*it2)->m_txRespFIFO.IsEmpty())) {
            cout<<"no request identified in the RR but there is a response which is core to core WB the ID is  "<<(*it2)->m_fifo_id<<endl;
            queusize = (*it2)->m_txRespFIFO.GetQueueSize();
            cout<<"queue size is  "<<queusize<<"  from  "<<(*it2)->m_fifo_id<<endl;
            if(queusize != 0) {
              cout<<"pendingQueueSize is regardless of the RR order and pricessing the consequences  "<<queusize<<endl;          
              for (int i = 0; i < queusize && PendingTxWBResp_temp == false;i++) {
                pendingWbMsg_temp = (*it2)->m_txRespFIFO.GetFrontElement();
                (*it2)->m_txRespFIFO.PopElement();
                if (PendingTxWBResp == false) {              
         ...
         // cout<<"the TX response is empty"<<endl;
        }
    ...
    */
        ...
        if (PendingTxWBResp == true && CheckOnly == false)
        {
            if (m_reza_log)
    ...
    { /*** Request Bus Arbiter for the RT Controller ***/
        ...
        if (m_PndReq)
        {
            ...
            m_PndReq = CheckPendingReq(m_reqCoreCnt, m_ReqBusMsg, false, m_PndOld);
       ...
       * Then advance RR order if didn't find anything, advance until you find something  */
        ...
        if (m_reza_log)
        {
            for (int u = 0; u < m_GlobalQueue->m_GlobalOldestQueue.GetQueueSize(); u++)
            {
                ...
                tempOldestReqMsgTemp = m_GlobalQueue->m_GlobalOldestQueue.GetFrontElement();
                m_GlobalQueue->m_GlobalOldestQueue.PopElement();
                cout << "AT " << u << " ADDR " << tempOldestReqMsgTemp.addr << "  ReqID  " << tempOldestReqMsgTemp.reqCoreId << "  MSGID " << tempOldestReqMsgTemp.msgId << "  AGENT  " << tempOldestReqMsgTemp.sharedCacheAgent << endl;
                m_GlobalQueue->m_GlobalOldestQueue.InsertElement(tempOldestReqMsgTemp);
            }
        }
        if (m_reza_log)
        ...
        if (m_reza_log)
        {
            for (int u = 0; u < m_GlobalQueue->m_GlobalReqFIFO.GetQueueSize(); u++)
            {
                ...
                tempNonOldestReqMsgTemp = m_GlobalQueue->m_GlobalReqFIFO.GetFrontElement();
                m_GlobalQueue->m_GlobalReqFIFO.PopElement();
                cout << "AT " << u << " ADDR " << tempNonOldestReqMsgTemp.addr << "  ReqID  " << tempNonOldestReqMsgTemp.reqCoreId << "  MSGID " << tempNonOldestReqMsgTemp.msgId << "  AGENT  " << tempNonOldestReqMsgTemp.sharedCacheAgent << endl;
                m_GlobalQueue->m_GlobalReqFIFO.InsertElement(tempNonOldestReqMsgTemp);
            }
        }
    ...
    void BusArbiter::adjustOldest(unsigned int coreIndex)
    {
        ...
        {
            if (m_reza_log)
                cout << " Requestor  " << coreIndex << "  Buffer not empty" << endl;
        ...
        {
            if (m_reza_log)
                cout << "Adding to the queue addr " << tempCandidate.addr << "  reqCoreID  " << tempCandidate.reqCoreId << endl;
    ...
    { /*** Response Bus Arbiter for the RT Controller ***/
        if (m_reza_log)
        ...
        if (m_PndResp || m_PndMemResp)
        {
            // a response from memory is sent
            if (m_PndMemResp)
            {
                if (m_reza_log)
                    cout << "||||||||||||||| In Response Arbiter  - Sending the Response from Shared Mem |||||||||||||||" << endl;
                m_PndMemResp = CheckPendingResp(m_PendResp, false, oldest_determine);
           ...
           */
                ...
                if (m_PendResp.msgId != 0)
                {
                    ...
                    if (isFinal(m_PendResp.addr, m_PendResp.msgId, m_PendResp.sharedCacheAgent))
                    {
                        ...
                        if (isOldest(m_PendResp.addr, m_PendResp.reqCoreId))
                        {
                            ...
                            {
                                if (m_GlobalQueue->m_GlobalRRQueue.at(h) == m_respCoreCnt)
                                {
            ...
            { // a response from core is sent
                if (m_reza_log)
                    cout << "||||||||||||||| In Response Arbiter  - Sending the Response from Core |||||||||||||||" << endl;
                //cout<<" In Response Arbiter  - Sending the Response Write Back from Core"<<endl;
                m_PndResp = CheckPendingWriteBackResp(false, m_PendResp, true);
                /** THE RESPONSE IS SENT FROM THE OWNER: WETHER IT IS THE MEMORY OR IN THIS CASE IT IS ONE OF THE CORES. IN THIS CASE
           * ALO IT SHOULD ALSO CONSIDER THAT REQUEST TO BE FINISHED FOR READ REQUEST FOR SURE
           **/
                // since the transaction is a write back and it is not finihsed at this point, the global RR queue should not change
                if (m_reza_log)
                    cout << "***************************** reponse from core is sent*******************************reqCoreID " << m_PendResp.reqCoreId << "  respCoreID  " << m_PendResp.respCoreId << endl;

                if (m_PendResp.msgId != 0)
                {
                    //cout<<"Manipulating the order and the global queue  "<<endl;
                    ...
                    cout << "shared agent " << m_PendResp.sharedCacheAgent << endl;
                    if (isFinal(m_PendResp.addr, m_PendResp.msgId, m_PendResp.sharedCacheAgent))
                    {
                        cout << "It is Final 2 " << endl;
                        if (isOldest(m_PendResp.addr, m_PendResp.reqCoreId))
                        { // 0- check first if it is the oldest of its requestor
                            // cout<<"It is oldest the order now is "<<m_GlobalQueue->m_GlobalRRQueue.at(0)<<endl;
                            // 1- If YES, adjust the order
                ...
                /**** We do not count for replacement here since the replacement MUST finish at the bank itself  ***/
                //  else {
                //    //cout<<"Manipulating the order and the global queue  for the replacement  "<<m_PendResp.respCoreId<<endl;
                //    if(isOldest(m_PendResp.addr,m_PendResp.respCoreId)) {  // 0- check first if it is the oldest of its requestor
                //      //cout<<"It is oldest the order now is "<<m_GlobalQueue->m_GlobalRRQueue.at(0)<<endl;
                //     // 1- If YES, adjust the order
                //     for(unsigned int h = 0; h < m_GlobalQueue->m_GlobalRRQueue.size() ; h++) {
                //       //cout<<"size is  "<<m_GlobalQueue->m_GlobalRRQueue.size()<<endl;
                //       if(m_GlobalQueue->m_GlobalRRQueue.at(h) == m_PendResp.respCoreId) {
                //         //cout<<"3"<<endl;
                //         m_GlobalQueue->m_GlobalRRQueue.erase(m_GlobalQueue->m_GlobalRRQueue.begin() + h);
                //       }
                //     }
                //     //cout<<"Order is adjusted and now is "<<m_GlobalQueue->m_GlobalRRQueue.at(0)<<"  the oldest queue size is  "<<m_GlobalQueue->m_GlobalOldestQueue.GetQueueSize()<<endl;
                //     // 2- If YES, remove this oldest request from oldest queue
                //     removeFromOldest(m_PendResp.addr,m_PendResp.respCoreId, true);
                //     if(m_reza_log) cout<<" the non oldest queue size "<<m_GlobalQueue->m_GlobalReqFIFO.GetQueueSize()<<endl;
                //     removeFromNonOldest(m_PendResp.addr,m_PendResp.respCoreId,true);
                //     if(m_reza_log) cout<<" the non oldest queue size after removing "<<m_GlobalQueue->m_GlobalReqFIFO.GetQueueSize()<<endl;
                //     //cout<<" it should be removed from oldest queue and now the size is  "<<m_GlobalQueue->m_GlobalOldestQueue.GetQueueSize()<<endl;
                //     // 3- adjust the oldest request
                //     adjustOldest(m_PendResp.respCoreId);
                //     if(m_reza_log) cout<<" the oldest shold be inserted if exist and now the size is  "<<m_GlobalQueue->m_GlobalOldestQueue.GetQueueSize()<<" the Order queue size is "<<m_GlobalQueue->m_GlobalRRQueue.size()<<endl;
                //     /*********************** shouldnt you remove it from the global request queue as well? ***************/
            }
        }
        ...
        m_PndMemResp = CheckPendingResp(m_PendResp, true, true);
        if (m_PndMemResp == false)
        {
            if (m_reza_log)
                cout << " CheckPendingResp - no oldest exists from shared memory " << endl;
            m_PndResp = CheckPendingWriteBackResp(true, m_PendResp, true);
            if (m_PndResp == false)
            {
                if (m_reza_log)
                    cout << " CheckPendingResp - no oldest exists from core write back " << endl;
                m_PndMemResp = CheckPendingResp(m_PendResp, true, false);
                if (m_PndMemResp == false)
                {
                    if (m_reza_log)
                        cout << " CheckPendingResp - no NON oldest exists from shared memory " << endl;
                    m_PndResp = CheckPendingWriteBackResp(true, m_PendResp, false);
                    if (m_PndResp)
                    {
                        oldest_determine = false;
                        if (m_reza_log)
                            cout << " ((((((((((((((((((((((((((((((((((((((((((((((((RESPONSE ARBITRATED From NON OLDET Core to another Core  ))))))))))))))))))))))))))))))))))))))))))))))))))))))" << endl;
                    }
                }
                else
                {
                    oldest_determine = false;
                    if (m_reza_log)
                        cout << " ((((((((((((((((((((((((((((((((((((((((((((((((RESPONSE ARBITRATED From NON OLDET SHARED MEMORY))))))))))))))))))))))))))))))))))))))))))))))))))))))" << endl;
                }
            }
            else
            {
                if (m_reza_log)
                    cout << " ((((((((((((((((((((((((((((((((((((((((((((((((RESPONSE ARBITRATED From a Core to another Core ))))))))))))))))))))))))))))))))))))))))))))))))))))))" << endl;
            }
        }
        else
        {
            if (m_reza_log)
                cout << " ((((((((((((((((((((((((((((((((((((((((((((((((RESPONSE ARBITRATED From OLDET SHARED MEMORY))))))))))))))))))))))))))))))))))))))))))))))))))))))" << endl;
        }

        // check service queue
        // wait Resp-TDM response slot
        if (m_PndResp == true || m_PndMemResp == true)
        {
            if (m_reza_log)
