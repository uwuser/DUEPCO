...
private:
    ...
    list<CacheXml> m_sharedCache;   // Modified to Accomodate Multi Shared Cache
...
public:
    ...
    list<CacheXml> GetSharedCache() {                    // Modified to Accomodate Multi Shared Cache
       return m_sharedCache;
    }

    void SetSharedCache(list<CacheXml> sharedCaches) {   // Modified to Accomodate Multi Shared Cache
      m_sharedCache = sharedCaches;
    }
    ...
    void LoadFromXml (TiXmlHandle root) {
       ...
       m_sharedCache        = list<CacheXml> ();  // Modified to Accomodate Multi Shared Cache
       ...
       if (rootPtr != NULL) {
          ...
          if (sharedCachesRootPtr) {                                                                // Modified to Accomodate Multi Shared Cache
            TiXmlElement* sharedCachePtr = sharedCachesRootPtr->FirstChildElement("sharedCache");   // Modified to Accomodate Multi Shared Cache
            for (; sharedCachePtr; sharedCachePtr = sharedCachePtr->NextSiblingElement()) {         // Modified to Accomodate Multi Shared Cache
              CacheXml newSharedCache;                                                              // Modified to Accomodate Multi Shared Cache
              TiXmlHandle sharedCacheHandle = TiXmlHandle(sharedCachePtr);                          // Modified to Accomodate Multi Shared Cache
              newSharedCache.LoadFromXml(sharedCacheHandle);                                        // Modified to Accomodate Multi Shared Cache
              m_sharedCache.push_back(newSharedCache);                                              // Modified to Accomodate Multi Shared Cache
            }
          }

          /* Original one shared cache code
