...
namespace ns3
{
  ...
  // bool BusArbiter::existinTxResp(BusIfFIFO::BusReqMsg Msg)
  // {
  //   BusIfFIFO::BusRespMsg Msg_temp;
  //   std::list<Ptr<BusIfFIFO>>::iterator itInverti = m_sharedCacheBusIfFIFO.begin();
  //   std::advance(itInverti, retrieveSharedCacheID(Msg.sharedCacheAgent));     
  //   for(unsigned int k = 0; k < (*itInverti)->m_txRespFIFO.GetQueueSize(); k++ ){
  //     Msg_temp = (*itInverti)->m_txRespFIFO.GetFrontElement();
  //     (*itInverti)->m_txRespFIFO.PopElement();
  //     if(Msg.addr == Msg_temp.addr && Msg.msgId == Msg_temp.msgId){
  //        (*itInverti)->m_txRespFIFO.InsertElement(Msg_temp);
  //       return true;         
  //     }
  //     (*itInverti)->m_txRespFIFO.InsertElement(Msg_temp);
  //   }  
  ...
  void BusArbiter::MSI_FcFsRespBus()
  {
       ...
       if (m_reza_log) cout<<"In MSI_FcFsRespBus 2"<<endl;
        ...
        for(int ii1=0; ii1<m_GlobalQueue->m_MsgType.GetQueueSize() && terminateii == false; ii1++){
          ...
          if(tempS.msgId == m_PendResp.msgId && tempS.addr == m_PendResp.addr) {
            if (m_reza_log) cout<<"FCFS Stage is changed for "<<tempS.msgId<<" to Response "<<endl;
    ...
    {
      if (m_reza_log)
        cout << "|||||||||||||||||||||||||||| HP RESP SCHEDULED TO SENT IN NEXT DELTA ||||||||||||||||||||||||||||||| "<< m_PendResp.msgId <<"  address "<<m_PendResp.addr<< endl;
          ...
          if(deadlineProgress_temp.msgId == deadlineProgress_temp_type.msgId && deadlineProgress_temp.addr == deadlineProgress_temp_type.addr) {
            ...
            { 
               if(deadlineProgress_temp.msgId == 0)
                cout<<"1Deadline of "<<deadlineProgress_temp_type.wbCoreId<<" msgID "<<deadlineProgress_temp_type.msgId<<" is "<<deadlineProgress_temp_type.associateDeadline<<endl;
              else 
                cout<<"1Deadline of "<<deadlineProgress_temp_type.reqCoreId<<" msgID "<<deadlineProgress_temp_type.msgId<<" is "<<deadlineProgress_temp_type.associateDeadline<<endl;  
            }           
  ...
  bool BusArbiter::WCLator() {
    ...
    if (m_reza_log) {
      ...
      for (int u = 0; u < m_GlobalQueue->m_GlobalOldestQueue.GetQueueSize(); u++)
      {
        ...
        m_GlobalQueue->m_GlobalOldestQueue.PopElement();
        cout << "AT " << u << " Oldest ADDR " << tempOldestReqMsgTemp.addr << "  ReqID  " << tempOldestReqMsgTemp.reqCoreId << "  wbID  " << tempOldestReqMsgTemp.wbCoreId << "  MSGID " << tempOldestReqMsgTemp.msgId << "  AGENT  " << tempOldestReqMsgTemp.sharedCacheAgent <<"  Arrival"  <<tempOldestReqMsgTemp.timestamp << endl;
        m_GlobalQueue->m_GlobalOldestQueue.InsertElement(tempOldestReqMsgTemp);
      }
      cout << "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ Global Service Queue Contains the Following  size is "<<m_GlobalQueue->m_GlobalReqFIFO.GetQueueSize()<<endl;  
      for (int u = 0; u < m_GlobalQueue->m_GlobalReqFIFO.GetQueueSize(); u++)
      {
        ...
        cout << "AT " << u << " ADDR " << tempNonOldestReqMsgTemp.addr << "  ReqID  " << tempNonOldestReqMsgTemp.reqCoreId << "  wbID  " << tempNonOldestReqMsgTemp.wbCoreId << "  MSGID " << tempNonOldestReqMsgTemp.msgId << "  AGENT  " << 
            tempNonOldestReqMsgTemp.sharedCacheAgent << " Cache line " <<retrieveSharedBankCacheLine(tempNonOldestReqMsgTemp.addr) <<"  Arrival"  <<tempNonOldestReqMsgTemp.timestamp << endl;
        m_GlobalQueue->m_GlobalReqFIFO.InsertElement(tempNonOldestReqMsgTemp);
      }
      if (m_GlobalQueue->m_MsgType.GetQueueSize() > 0 )
      {
        ...
        for (int u = 0; u < m_GlobalQueue->m_MsgType.GetQueueSize(); u++)
        {
          ...
          cout << "AT " << u << " ADDR " << tempOldestReqMsgTemp_mtype.addr << "  ReqID  " << tempOldestReqMsgTemp_mtype.reqCoreId << "  wbID  " << tempOldestReqMsgTemp_mtype.wbCoreId << "  MSGID " << tempOldestReqMsgTemp_mtype.msgId << "  AGENT  " <<
              tempOldestReqMsgTemp_mtype.sharedCacheAgent << " curr stage "<< tempOldestReqMsgTemp_mtype.currStage<< " Cache line " <<retrieveSharedBankCacheLine(tempOldestReqMsgTemp_mtype.addr)<<" ORder of Arb "<<tempOldestReqMsgTemp_mtype.orderofArbitration <<"  Arrival"  <<tempOldestReqMsgTemp_mtype.timestamp << endl;
    ...
    // if (m_reza_log) cout << "In BusArbiter::WCLator   " << m_arbiCycle <<" REQ Bus Blocked "<<m_GlobalQueue->m_busArbBlock<<" RESPONSE Bus Blocked "<<m_GlobalQueue->m_respArbBlock<< endl; 
    // if (m_reza_log) {
    ...
    for (std::list<Ptr<BusIfFIFO>>::iterator itk = m_busIfFIFO.begin(); itk != m_busIfFIFO.end(); itk++)
    {
      ...
      if ((*itk)->m_txMsgFIFO.IsEmpty() == false)
      {
        ...
        //cout<<" for shared agent "<<txMsg_k.sharedCacheAgent<<" address "<<txMsg_k.addr<<" cache line "<<retrieveSharedBankCacheLine(txMsg_k.addr)<<" from "<<(*itk)->m_fifo_id<<endl;   
        for (int itkM = 0; itkM < m_GlobalQueue->m_MsgType.GetQueueSize(); itkM++)
        {
          ...
          if(((retrieveSharedBankCacheLine(txMsg_k.addr) == retrieveSharedBankCacheLine(tempk.addr)) && (txMsg_k.sharedCacheAgent == tempk.sharedCacheAgent) && (tempk.msgId != 0) && (!isOldest(tempk.addr,tempk.reqCoreId))) ||
            ...
            //cout<<" XXXX "<<tempk.sharedCacheAgent<<" address "<<tempk.addr<<" cache line "<<retrieveSharedBankCacheLine(tempk.addr)<<" from "<<tempk.reqCoreId<<" ID "<<tempk.msgId<<endl;
            iSameLine++;
            //cout<<"INCREMENT  "<<iSameLine<<endl;
            if((txMsg_k.msgId == tempk.msgId) && (txMsg_k.addr == tempk.addr) && (txMsg_k.reqCoreId == tempk.reqCoreId)){
               iSameLine--; 
               //cout<<"Is same "<<iSameLine<<endl;
            }
    ...
    if(!m_GlobalQueue->m_GlobalOldestQueue.IsEmpty()){    
      for (int it5 = 0; it5 < m_GlobalQueue->m_GlobalOldestQueue.GetQueueSize(); it5++) {      
            ...
            for (int it9 = 0; it9 < tempFromOldest.GetQueueSize() && nextInMemType == false; it9++) {
              ...
              if(((tempOldestMsgQueueWCLator_FromMemType_local.msgId == 0 && tempOldestMsgQueueWCLator_FromMemType_local.wbCoreId == tempOrderFront) 
                ...
                for(int ii2=0; ii2<m_GlobalQueue->m_MsgType.GetQueueSize() && nextInMemType == false; ii2++) {               
                  ...
                  //  cout<<"found in the memtype  from the oldest order of arbitration "<<tempOldestMsgQueueWCLator_FromMemType.orderofArbitration<<" bank agent "<<tempOldestMsgQueueWCLator_FromMemType.sharedCacheAgent<<"  id is "<<tempOldestMsgQueueWCLator_FromMemType.msgId<<endl;
        ...
        if (m_reza_log) cout<<"For ReqID "<<WCLatorReqID<<" and msgID "<<tempOldestMsgQueueWCLator.msgId<<" the RR order is "<<currentOrder<<" order of arbitration "<<tempOldestMsgQueueWCLator_FromMemType.orderofArbitration<<" deadline "<<deadline<<" Final WCLator in Arbiter Done with Ka "<<k_a<<" Kb "<<k_b<<" Kc "<<k_c<<" Kd "<<k_d<<" Ke "<<k_e<<endl;
        ...
        if(tempOldestMsgQueueWCLator_FromMemType.orderDetermined){
         ...
         // cout<<"1  "<<tempOldestMsgQueueWCLator_FromMemType.orderofArbitration<<"  curr "<<tempOldestMsgQueueWCLator_FromMemType.currStage<<" resp block "<<m_GlobalQueue->m_respArbBlock<< " clk  "<<m_arbiCycle<< endl;
          ...
          case 0:
            ...
            if(tempOldestMsgQueueWCLator_FromMemType.currStage == "REQ") {            
              ...
              //cout<<"1  id "<<(*itInvert)->m_fifo_id<<endl;
              // check if there is anything that HPA can process on same BANK from RxResp from lower priority   
             ...
             //cout<<"2"<<endl;
              ...
              //cout<<"3  empty "<<(*itInvert)->m_rxMsgFIFO.IsEmpty()<<" bank clock  "<<(*itInvert)->m_bankArbBlock<<endl;
              ...
              if ((*itInvert)->m_rxRespFIFO.IsEmpty() == false && ((*itInvert)->m_bankArbBlock == 2 ))
              {
                ...
                if (RxResp_HP.msgId != 0 && isOldest(RxResp_HP.addr,RxResp_HP.reqCoreId)) {
                  ...
                  if((tempOldestMsgQueueWCLator_FromMemType.msgId == 0 &&  isHigherPrio(tempOldestMsgQueueWCLator_FromMemType.wbCoreId, RxResp_HP.respCoreId) == true) ||
                      ...
                      (tempOldestMsgQueueWCLator_FromMemType.msgId != 0  && (tempOldestMsgQueueWCLator_FromMemType.reqCoreId == RxResp_HP.respCoreId))  ||
                       (tempOldestMsgQueueWCLator_FromMemType.msgId == 0  && (tempOldestMsgQueueWCLator_FromMemType.wbCoreId == RxResp_HP.respCoreId))) {
                         //cout<<"7 RESPONSE "<<endl;
                    addedLatency =  m_sharedbankclks;
                  }
                }
                else {
                  //cout<<"6 RESPONSE "<<endl;
                  addedLatency =  m_sharedbankclks;
                }                  
              }
        ...
        if (m_reza_log) cout<<"After retaliation "<<k_a<<" added latency "<<addedLatency<<endl;
        latency = 0;
        ...
        if((tempOldestMsgQueueWCLator_FromMemType.orderDetermined == true) && (tempOldestMsgQueueWCLator_FromMemType.associateDeadline_final==true)){
          ...
          case 0:
            ...
            if(tempOldestMsgQueueWCLator_FromMemType.currStage == "REQ") {
              ...
              if (m_reza_log) cout<<"Latency Est. 1 for ID "<<tempOldestMsgQueueWCLator_FromMemType.msgId<<"  Addr  "<<tempOldestMsgQueueWCLator_FromMemType.addr<<endl;
              ...
              if (m_reza_log) cout<<"Latency Est. 2 for ID "<<tempOldestMsgQueueWCLator_FromMemType.msgId<<"  Addr  "<<tempOldestMsgQueueWCLator_FromMemType.addr<<endl;
              ...
              if (m_reza_log) cout<<"Latency Est. 3 for ID "<<tempOldestMsgQueueWCLator_FromMemType.msgId<<"  Addr  "<<tempOldestMsgQueueWCLator_FromMemType.addr<<endl;
              // The request finished its last stage
              latency = addedLatency + 1;
            }
            else {    
              if (m_reza_log) cout<<"the current is "<<tempOldestMsgQueueWCLator_FromMemType.currStage<<endl;          
              if (m_reza_log) cout<<"Latency Est. 4 for ID "<<tempOldestMsgQueueWCLator_FromMemType.msgId<<"  Addr  "<<tempOldestMsgQueueWCLator_FromMemType.addr<<endl;              
              //abort();
          ...
          case 1:
            /* REQ:RESPONSE:BANK */
            if(tempOldestMsgQueueWCLator_FromMemType.currStage == "REQ") {
              if (m_reza_log) cout<<"Latency Est. 5 for ID "<<tempOldestMsgQueueWCLator_FromMemType.msgId<<"  Addr  "<<tempOldestMsgQueueWCLator_FromMemType.addr<<endl;
              // The request finished proceed REQ stage
              latency = addedLatency +  m_GlobalQueue->m_respArbBlock + B_2 + m_sharedbankclks + m_respclks + (k_a) * max(m_sharedbankclks,m_respclks) 
                  + (k_b + k_d + k_e) * (m_respclks) + k_c * max(m_sharedbankclks,m_respclks); 
            }
            else if(tempOldestMsgQueueWCLator_FromMemType.currStage == "RESPONSE"){
              if (m_reza_log) cout<<"Latency Est. 6 for ID "<<tempOldestMsgQueueWCLator_FromMemType.msgId<<"  Addr  "<<tempOldestMsgQueueWCLator_FromMemType.addr<<endl;
              ...
              if (m_reza_log) cout<<"Latency Est. 7 for ID "<<tempOldestMsgQueueWCLator_FromMemType.msgId<<"  Addr  "<<tempOldestMsgQueueWCLator_FromMemType.addr<<endl;
              // The request finished its last stage
              latency = addedLatency + 1;
            }
            else {
              if (m_reza_log) cout<<"Latency Est. 8 for ID "<<tempOldestMsgQueueWCLator_FromMemType.msgId<<"  Addr  "<<tempOldestMsgQueueWCLator_FromMemType.addr<<endl;
              //abort();
          ...
          case 2:
            /* REQ:RESP */
            if(tempOldestMsgQueueWCLator_FromMemType.currStage == "REQ") {
              if (m_reza_log) cout<<"Latency Est. 9 for ID "<<tempOldestMsgQueueWCLator_FromMemType.msgId<<"  Addr  "<<tempOldestMsgQueueWCLator_FromMemType.addr<<endl;;
              // The request finished proceed REQ stage
              latency = addedLatency + m_GlobalQueue->m_respArbBlock + m_sharedbankclks + m_respclks + (k_a) * max(m_sharedbankclks,m_respclks) 
                  + (k_b + k_d + k_e) * (m_respclks) + k_c * max(m_sharedbankclks,m_respclks);
            }
            else if(tempOldestMsgQueueWCLator_FromMemType.currStage == "RESPONSE"){
              if (m_reza_log) cout<<"Latency Est. 10 for ID "<<tempOldestMsgQueueWCLator_FromMemType.msgId<<"  Addr  "<<tempOldestMsgQueueWCLator_FromMemType.addr<<endl;
              // The request finished its last stage
              latency = addedLatency + 1;
            }
            else{
              if (m_reza_log) cout<<"Latency Est. 11 for ID "<<tempOldestMsgQueueWCLator_FromMemType.msgId<<"  Addr  "<<tempOldestMsgQueueWCLator_FromMemType.addr<<endl;
              //abort();
              // The request did not proceed into any stages
              latency = addedLatency + B_1 + B_2 + B_3 + m_reqclks + m_sharedbankclks + m_respclks + (k_a) * max(m_reqclks,max(m_sharedbankclks,m_respclks))
                  + (k_b + k_d + k_e) * (m_reqclks + m_respclks) + k_c * max(m_sharedbankclks,m_respclks); 
          ...
          //cout<<"6"<<endl;
         if (m_reza_log)  cout<<"order is not determined at all meaning that it is waiting for its REQ to issue on the bus. "<<endl;      
          if (m_reza_log) cout<<"Latency Est. 12 for ID "<<tempOldestMsgQueueWCLator_FromMemType.msgId<<"  Addr  "<<tempOldestMsgQueueWCLator_FromMemType.addr<<endl;
          // The request did not proceed into any stages
          latency = addedLatency + B_1 + B_2 + B_3 + m_reqclks + m_sharedbankclks + m_respclks + (k_a) * max(m_reqclks,max(m_sharedbankclks,m_respclks))
              + (k_b + k_d + k_e) * (m_reqclks + m_respclks) + k_c * max(m_sharedbankclks,m_respclks); 
        }

        if (m_reza_log) cout<<"############### "<<WCLatorReqID<<"  WCLator in Arbiter Done with Latency "<<latency<<" and the assosicate deadline is "<<tempOldestMsgQueueWCLator_FromMemType.associateDeadline<<endl;
        ...
        if (msg.reqCoreId == (*it2)->m_fifo_id)
        {
          if (!(*it2)->m_rxRespFIFO.IsFull())
          {
            if(msg.respCoreId < 10){
              ...
              if(msg.respCoreId == 0)       m_GlobalQueue->m_Core2CoreTransfer_0++;
              else if (msg.respCoreId == 1) m_GlobalQueue->m_Core2CoreTransfer_1++;
              else if (msg.respCoreId == 2) m_GlobalQueue->m_Core2CoreTransfer_2++;
              else if (msg.respCoreId == 3) m_GlobalQueue->m_Core2CoreTransfer_3++;
              else if (msg.respCoreId == 4) m_GlobalQueue->m_Core2CoreTransfer_4++;
              else if (msg.respCoreId == 5) m_GlobalQueue->m_Core2CoreTransfer_5++;
              else if (msg.respCoreId == 6) m_GlobalQueue->m_Core2CoreTransfer_6++;
              else if (msg.respCoreId == 7) m_GlobalQueue->m_Core2CoreTransfer_7++;
            }
            else{
              m_GlobalQueue->m_SharedBankTransfer++; 
              if(msg.reqCoreId == 0)      m_GlobalQueue->m_SharedBankTransfer_0++;
              else if (msg.reqCoreId == 1) m_GlobalQueue->m_SharedBankTransfer_1++;
              else if (msg.reqCoreId == 2) m_GlobalQueue->m_SharedBankTransfer_2++;
              else if (msg.reqCoreId == 3) m_GlobalQueue->m_SharedBankTransfer_3++;
              else if (msg.reqCoreId == 4) m_GlobalQueue->m_SharedBankTransfer_4++;
              else if (msg.reqCoreId == 5) m_GlobalQueue->m_SharedBankTransfer_5++;
              else if (msg.reqCoreId == 6) m_GlobalQueue->m_SharedBankTransfer_6++;
              else if (msg.reqCoreId == 7) m_GlobalQueue->m_SharedBankTransfer_7++; 
            }
  ...
  unsigned int BusArbiter::retrieveSharedCacheID(uint64_t id)
  {
    ...
    else if (id == 15) return 5;
    else if (id == 16) return 6;
    else if (id == 17) return 7;  
    else if (id == 18) return 8;
    else if (id == 19) return 9;
    else if (id == 20) return 10;
    else if (id == 21) return 11;
    else if (id == 22) return 12;
    else if (id == 23) return 13;
    else if (id == 24) return 14;
    else if (id == 25) return 15;    
  ...
  {
    if (m_reza_log) cout<<"CheckPendingResp ChkOnly "<<ChkOnly<<endl;
    ...
    if((ChkOnly && (m_delaycycle == 2)) || ((ChkOnly) && (m_arbiCycle == m_delaycyclereg))) {
      //cout<<"m_delaycycle becomes 0 "<<endl;
      m_delaycycle = 0;
      return false;
    }
    ...
    if(!ChkOnly) {
      //cout<<"1"<<endl;
      if(m_delaycycle == 1) {        
        //cout<<"m_delaycycle becomes 2 "<<endl;
        m_delaycyclereg = m_arbiCycle;
        m_delaycycle = 2;
      }
      issueTemp = txResp;
      if(issueTemp.msgId == 0){
        //cout<<"2  issueTemp.respCoreId  "<<issueTemp.respCoreId<<" req ID "<<issueTemp.reqCoreId<<endl;
        ...
        for(int itc1 = 0; itc1 < pendingQueueSize ; itc1++) {
          ...
          if(pendingWbMsg_3.msgId == issueTemp.msgId && pendingWbMsg_3.addr == issueTemp.addr){
            ...
            //cout<<"found and issued ID "<< pendingWbMsg_3.msgId<<" adr "<<pendingWbMsg_3.addr<<endl;
          }
          else{
            //cout<<"5"<<endl;
        ...
        if(issueTemp.respCoreId < 10){
          ...
          for (int itc1 = 0; itc1 < pendingQueueSize; itc1++){
            ...
            if(pendingWbMsg_3.msgId == issueTemp.msgId && pendingWbMsg_3.addr == issueTemp.addr){
              PendingTxResp = true;   
              //cout<<"found and issued in shared bank ID "<< pendingWbMsg_3.msgId<<" adr "<<pendingWbMsg_3.addr<<endl;
            }
            else{
              (*itc)->m_txRespFIFO.InsertElement(pendingWbMsg_3);
            }
        ...
        if (m_ServQueueMsg.reqCoreId == m_respCoreCnt)
        {
          ...
          for (int i = 0; i < pendingQueueSize && PendingTxResp == false; i++)
          {
            ...
            if ((pendingWbMsg_1.reqCoreId == m_respCoreCnt) && (pendingWbMsg_1.addr == m_ServQueueMsg.addr))
            {
              ...
              {
                m_delaycycle = 1;
                //cout<<"m_delaycycle becomes 1 "<<endl;
          ...
          //cout<<"1 Nothing found in the RESP buffer of bank"<<endl;
          if(((oldest) && (PendingTxResp == false) && (ChkOnly==true)) || ((!oldest) && (PendingTxResp == false) && (ChkOnly==false))) {
            bool PendingTxRespg = false;   
            //cout<<"2 Nothing found in the RESP buffer of bank  PendingTxResp  "<< PendingTxResp <<"  chekonly "<<ChkOnly<<"  oldest  "<<oldest<<endl;
            for(int itg = 0; itg < m_GlobalQueue->m_GlobalReqFIFO.GetQueueSize() && PendingTxRespg == false ; itg++){
              ...
              //cout<<"checking id "<<itgReq.msgId<<" in the global FIFO"<<endl;
              m_GlobalQueue->m_GlobalReqFIFO.PopElement();
              ...
              if(((retrieveSharedBankCacheLine(m_ServQueueMsg.addr) == retrieveSharedBankCacheLine(itgReq.addr)) && (m_ServQueueMsg.sharedCacheAgent == itgReq.sharedCacheAgent) && 
                ...
                for (int i = 0; i < pendingQueueSize && PendingTxRespg == false; i++)
                {
                  ...
                  if((pendingWbMsg_1.msgId == itgReq.msgId) && (pendingWbMsg_1.addr == itgReq.addr)){
                    ...
                    //cout<<" 1 - found in the shared bank and the ID is "<<pendingWbMsg_1.msgId<< "now we will schedule this even not oldest and lower priority reqID "<<pendingWbMsg_1.reqCoreId<<" resp "<<pendingWbMsg_1.respCoreId<<endl;
                    itx = itg1;
                    mode = 1;
                    selected = true;
                ...
                ///cout<<"3 Nothing found in the RESP buffer of bank"<<endl;
                ...
                {
                  //cout<<"check core  "<<(*itg2)->m_fifo_id<<endl;
                  ...
                  for (int i = 0; i < pendingQueueSize && PendingTxRespg == false; i++)
                  {
                    ...
                    //cout<<"the request in the TX resp id is "<<pendingWbMsg_1.msgId<<"  adr "<<pendingWbMsg_1.addr<<endl;
                    if((pendingWbMsg_1.msgId == itgReq.msgId) && (pendingWbMsg_1.addr == itgReq.addr)){
                      //cout<<"Ok"<<endl;
                      PendingTxRespg = true;
                      PendingTxResp = true;
                      ...
                      //cout<<"2 - found in the shared bank and the ID is "<<pendingWbMsg_1.msgId<< "now we will schedule this even not oldest and lower priority reqID "<<pendingWbMsg_1.reqCoreId<<
                       //   " resp "<<pendingWbMsg_1.respCoreId<<"  from core "<<(*itg2)->m_fifo_id <<endl;
                      itx = itg2;
                      mode = 1;
                      selected = true;
                      selectedPrioInvert = true;
         ...
         // cout<<"nothing found for RR order"<<endl;
    ...
    //cout<<"now sending msgID "<<txResp.msgId<<" adress "<<txResp.addr<<endl;
  ...
  bool BusArbiter::CheckPendingFCFSResp(BusIfFIFO::BusRespMsg &txMsg, bool ChkOnly = true)
  {
    ...
    for (std::list<Ptr<BusIfFIFO>>::iterator it1 = m_busIfFIFO.begin(); it1 != m_busIfFIFO.end(); it1++)
    {
      if ((*it1)->m_txRespFIFO.IsEmpty() == false)
        {
          if (m_reza_log) cout<<"Size of TX RESP of  "<<(*it1)->m_fifo_id<<" is "<<(*it1)->m_txRespFIFO.GetQueueSize()<<endl;
        }

    }

    if(ChkOnly) {
      if (m_reza_log) cout<<"CheckPendingFCFSResp 1"<<endl;
      for (std::list<Ptr<BusIfFIFO>>::iterator it1 = m_busIfFIFO.begin(); it1 != m_busIfFIFO.end(); it1++)
      {
        if ((*it1)->m_txRespFIFO.IsEmpty() == false)
        {
          txMsg = (*it1)->m_txRespFIFO.GetFrontElement();
          if (m_reza_log) cout<<m_arbiCycle<<" req "<<txMsg.reqCoreId<<"  resp  "<<txMsg.respCoreId<<"  the arrival is "<<txMsg.timestamp<<endl;
          if((txMsg.msgId != 0 && txMsg.reqCoreId == 1) || (txMsg.msgId == 0 && txMsg.respCoreId == 1) 
              || (txMsg.respCoreId < 10 && txMsg.reqCoreId < 10 && txMsg.respCoreId == 1)
              || (txMsg.respCoreId < 10 && txMsg.reqCoreId < 10 && txMsg.reqCoreId == 1)) {
          ...
          if ((PendingTxReq == false) || arrivalTime > txMsg.timestamp)
          {
            if (m_reza_log) cout<<"CheckPendingFCFSResp 2 "<<txMsg.timestamp<<endl;
            arrivalTime = txMsg.timestamp;
            TargetCore = core_idx;
          }
          m_delaycycle = 1;
      ...
      for (std::list<Ptr<BusIfFIFO>>::iterator it2 = m_sharedCacheBusIfFIFO.begin(); it2 != m_sharedCacheBusIfFIFO.end(); it2++)
      {
        ...
        if ((*it2)->m_txRespFIFO.IsEmpty() == false)
        {
          ...
          //cout<<"  th seize "<<(*it2)->m_txRespFIFO.GetQueueSize()<<endl;
          ...
          if(!(*it2)->m_txRespFIFO.IsEmpty()){
            for(int i = 0; i< (*it2)->m_txRespFIFO.GetQueueSize(); i++){
              ...
              //cout<<" the msg ID "<<txMsgMem_temp_1.msgId<<"  arrival "<<txMsgMem_temp_1.timestamp<<endl;
          ...
          if (m_reza_log) cout<<"The oldest of "<<(*it2)->m_fifo_id<<"  has the address "<<txMsgMem_temp.addr<<"  and arrival is  "<<txMsgMem_temp.timestamp<<
                  "  ID "<<txMsgMem_temp.msgId<<" from reqID "<<txMsgMem_temp.reqCoreId<<"  and resp ID "<<txMsgMem_temp.respCoreId<<endl;

          if ((PendingTxReq == false) || arrivalTime > txMsgMem_temp.timestamp)
          {
            if (m_reza_log) cout<<"Chosed"<<endl;
      ...
      if (m_reza_log) cout<<"CheckPendingFCFSResp 3"<<endl;
      if (TargetCore >= 10)
      {
        if (m_reza_log) cout<<"CheckPendingFCFSResp 8"<<endl;
        txMsg = txMsgMem;
        // if (ChkOnly == false)
        // {
      ...
      {
        if (m_reza_log) cout<<"CheckPendingFCFSResp 9"<<endl;
        //cout<<"here 1"<<endl;
    ...
    for (int itr = 0; itr < queueSize; itr++)
    {
          ...
          (replacement == true && tempOldestReqMsg.addr == adr && tempOldestReqMsg.wbCoreId == coreIndex))
      {
        //if (m_reza_log){
          ...
          cout<<"                 Core The Oldest MsgID "<<tempOldestReqMsg.msgId<<" reQID "<<coreIndex<<" wbID "<<tempOldestReqMsg.wbCoreId<<" Terminated @ "<<m_arbiCycle<<" Arrival @ "<<tempOldestReqMsg.becameOldest<<" Latency: "<<
                m_arbiCycle - tempOldestReqMsg.becameOldest<<"  The WC is "<<m_wcArbiter<<endl; 

          }
          */

          if(m_arbiCycle - tempOldestReqMsg.becameOldest > 80) {
            if(m_arbiCycle - tempOldestReqMsg.becameOldest > 473)          
              cout<<"473"<<endl;
            else 
              cout<<m_arbiCycle - tempOldestReqMsg.becameOldest<<endl;
  ...
  bool BusArbiter::isFinal(uint64_t adr, unsigned int messageID, uint64_t sharedID)
  {
    ...
    {
      //cout<<"id is "<<(*it7)->m_fifo_id<<"  size is  "<<(*it7)->m_rxRespFIFO.GetQueueSize()<<endl;
      for (int itr_temp = 0; itr_temp < (*it7)->m_rxRespFIFO.GetQueueSize(); itr_temp++)
      {
        BusRespMsg_RXtemp = (*it7)->m_rxRespFIFO.GetFrontElement();
        //cout<<"BusRespMsg_RXtemp id  "<<BusRespMsg_RXtemp.msgId<<endl;
  ...
  void BusArbiter::RR_RT_RespBus()
  {
      ...
      if (m_PndResp)
      {
        ...
        if (m_reza_log)
          cout << "||||||||||||||| In Response Arbiter  - Sending the Response |||||||||||||||   msgID  " << m_PendResp.msgId << "addr "<< m_PendResp.addr <<" req "<<m_PendResp.reqCoreId<<" resp "<<m_PendResp.respCoreId << endl;
        //cout<<"mode "<<mode<<endl;
        // if(mode == 1){      
  ...
  {
    if (m_reza_log) 
      cout << "In BusArbiter::CycleAdvance   " << m_arbiCycle <<" REQ Bus Blocked "<<m_GlobalQueue->m_busArbBlock<<" RESPONSE Bus Blocked "<<m_GlobalQueue->m_respArbBlock<< endl; 
    if (m_reza_log) {
      for (std::list<Ptr<BusIfFIFO>>::iterator it13 = m_sharedCacheBusIfFIFO.begin(); it13 != m_sharedCacheBusIfFIFO.end(); it13++)
      {
        cout << "For Bank "<<(*it13)->m_fifo_id<<" Bank Block "<<(*it13)->m_bankArbBlock<<endl; 
    ...
    // if(m_arbiCycle > 33000 )
    //    m_reza_log = true;
    // if(m_arbiCycle == 10000)
    //   abort();
