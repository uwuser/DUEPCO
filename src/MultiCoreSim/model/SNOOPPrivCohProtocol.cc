  ...
  void SNOOPPrivCohProtocol::ChkReqBusEvent()
  {
    ...
    if (!m_busIfFIFO->m_rxMsgFIFO.IsEmpty())
    {
      ...
      case SNOOPPrivCohTrans::GetSTrans:
        ...
        m_eventList.busReqEvent = (reqCoreId == m_coreId) ? SNOOPPrivReqBusEvent::OwnGetS : SNOOPPrivReqBusEvent::OtherGetS;
        break;
      case SNOOPPrivCohTrans::GetMTrans:

        m_eventList.busReqEvent = (reqCoreId == m_coreId) ? SNOOPPrivReqBusEvent::OwnGetM : SNOOPPrivReqBusEvent::OtherGetM;
        break;
      case SNOOPPrivCohTrans::UpgTrans:

        m_eventList.busReqEvent = (reqCoreId == m_coreId) ? SNOOPPrivReqBusEvent::OwnUpg : SNOOPPrivReqBusEvent::OtherUpg;
        break;
      case SNOOPPrivCohTrans::PutMTrans:

        m_eventList.busReqEvent = (wbCoreId == m_coreId) ? SNOOPPrivReqBusEvent::OwnPutM : SNOOPPrivReqBusEvent::OtherPutM;
        break;
      case SNOOPPrivCohTrans::PutSTrans:

        m_eventList.busReqEvent = (wbCoreId == m_coreId) ? SNOOPPrivReqBusEvent::OwnPutS : SNOOPPrivReqBusEvent::OtherPutS;
        break;
      case SNOOPPrivCohTrans::ExclTrans:

        m_eventList.busReqEvent = (reqCoreId == m_coreId) ? SNOOPPrivReqBusEvent::OwnExclTrans : SNOOPPrivReqBusEvent::OtherExclTrans;
        break;
    ...
    if (!m_busIfFIFO->m_rxRespFIFO.IsEmpty())
    {
      ...
      if (m_cache2Cache == false)
      {
        m_eventList.busRespEvent = (m_msgList.busRespMsg.reqCoreId == m_coreId &&
                                    m_msgList.busRespMsg.respCoreId == m_msgList.busRespMsg.sharedCacheAgent)
                                       ? SNOOPPrivRespBusEvent::OwnDataResp
                                       : SNOOPPrivRespBusEvent::OtherDataResp;
      }
      else
      {
        m_eventList.busRespEvent = (m_msgList.busRespMsg.reqCoreId == m_coreId) ? SNOOPPrivRespBusEvent::OwnDataResp : SNOOPPrivRespBusEvent::OtherDataResp;
      }
    }
    else
    {
